...
async function getOrCreateConversation(): Promise<string> {
  const url  = BASE44_BASE + "/" + BASE44_AGENT_ID + "/conversations/default";
  const resp = await fetch(url, {
    method: "POST",
    headers: { "api_key": BASE44_API_KEY, "Content-Type": "application/json" },
  });
  if (!resp.ok) throw new Error("Conv init failed: " + resp.status);
  const d = await resp.json() as { id?: string; conversation_id?: string };
  return d.id || d.conversation_id || "default";
}

async function sendMessage(convId: string, message: string): Promise<string> {
  const url  = BASE44_BASE + "/" + BASE44_AGENT_ID + "/conversations/" + convId + "/messages";
  const resp = await fetch(url, {
    method: "POST",
    headers: { "api_key": BASE44_API_KEY, "Content-Type": "application/json" },
  ...
  if (!resp.ok) {
    ...
    throw new Error("Message failed: " + resp.status + " " + e.slice(0,200));
  }
  const d = await resp.json() as { content?: string; message?: string; response?: string; text?: string };
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const fullMsg = body.context
      ? "[Context: " + body.context + "] " + userMessage
      : userMessage;
